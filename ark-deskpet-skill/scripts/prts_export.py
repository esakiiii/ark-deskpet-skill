        ...
        try:
            ...
            for anim in ANIMATIONS:
                ...
                with page.expect_download(timeout=120000) as info:
                    download.click()
                dl = info.value
                # ``suggested_filename`` is a property in current Playwright,
                # but historical builds exposed it as a method. Accept both.
                suggested = dl.suggested_filename
                if callable(suggested):
                    suggested = suggested()
                ext = os.path.splitext(suggested)[1] or ".webm"
